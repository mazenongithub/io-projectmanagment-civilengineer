...
class MaterialCalender {
    ...
    showicon() {
        ...
        const dropIcon = pm.getdropicon.call(this)
        if (this.state.completioncalender) {
            return (
                <button style={{ ...styles.generalButton, ...removeIcon }} onClick={() => { this.setState({ completioncalender: false }) }}>{removeIconSmall()} </button>
            )
        } else {
            return (
                <button style={{ ...styles.generalButton, ...dropIcon }} onClick={() => { this.setState({ completioncalender: true }) }}>{dropDateIcon()} </button>
            )
        }
